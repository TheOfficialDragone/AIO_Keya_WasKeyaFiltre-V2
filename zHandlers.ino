...
void readBNO()
{
    if (bno08x.dataAvailable() == true)
    {
        ...
        */
        ...
        if (norm < 1e-6f) return;   // all-zero quaternion (sensor reset / I2C glitch) — skip
        dqw = dqw / norm;
        dqx = dqx / norm;
        dqy = dqy / norm;
        dqz = dqz / norm;
        ...
        {
            roll = asinf(t2) * RAD_TO_DEG_X_10;
            pitch = atan2f(t0, t1) * RAD_TO_DEG_X_10;
        }
        else
        {
            pitch = asinf(t2) * RAD_TO_DEG_X_10;
            roll = atan2f(t0, t1) * RAD_TO_DEG_X_10;
        }
...
bool handleEmaSerialCommand(const String &cmd)
{
    ...
    if (cmd.startsWith("EY"))
    {
        ...
        {
            Serial.println(F("EY: hors plage [0.0-1.0]"));
            return true;
        }
        emaYawAlpha = val;
        emaYawInit = false;
        ...
        Serial.println(emaYawAlpha, 3);
        if (emaYawAlpha == 0.0f)
            Serial.println(F("  (filtre YAW DESACTIVE)"));
        return true;
    }
    ...
    if (cmd.startsWith("ER"))
    {
        ...
        if (val < 0.0f || val > 1.0f)
        {
            Serial.println(F("ER: hors plage [0.0-1.0]"));
            return true;
        }
        emaRollAlpha = val;
        emaRollInit = false;
        ...
        if (emaRollAlpha == 0.0f)
            Serial.println(F("  (filtre ROLL DESACTIVE)"));
        return true;
    }
    ...
    if (cmd.startsWith("EP"))
    {
        ...
        if (val < 0.0f || val > 1.0f)
        {
            Serial.println(F("EP: hors plage [0.0-1.0]"));
            return true;
        }
        emaPitchAlpha = val;
        emaPitchInit = false;
        ...
        if (emaPitchAlpha == 0.0f)
            Serial.println(F("  (filtre PITCH DESACTIVE)"));
        return true;
    }
    ...
    if (cmd.startsWith("ES"))
    {
        ...
        {
            Serial.println(F("ES: hors plage [0.0-20.0 km/h]"));
            return true;
        }
        emaStopKmh = val;
        Serial.print(F("EMA Stop seuil -> "));
        Serial.print(emaStopKmh, 1);
        Serial.println(F(" km/h"));
        if (emaStopKmh == 0.0f)
            Serial.println(F("  (filtrage permanent, pas de reset a l'arret)"));
        return true;
    }
